...
RSpec.describe "MultiVolumeWorksController", type: :feature do
  ...
  it "Logged in user can follow link to edit multi-volume work" do
    visit polymorphic_path [multi_volume_work]
    click_link 'Edit This Multi Volume Work'
    expect(page).to have_text('Manage Your Work')
  end

  it "User can edit a multi-volume work", vcr: { cassette_name: "ezid" } do
...
" has no members associated with it.')
    ...
    have_current_path("/concern/parent/#{multi_volume_work.id}"\
                      "/scanned_resources/",
                      only_path: true)
