    ...
    private
      ...
      def update_or_create_error(err)
        ...
        when JSONLDRecord::MissingRemoteRecordError
          error_ui_text = "The entered ID did not match any #{I18n.t('services.metadata')} records."
          error_log_text = "No #{I18n.t('services.metadata')} record found: #{curation_concern.source_metadata_identifier}: #{err}"
        when IuMetadata::MarcRecord::MarcParsingError
          error_ui_text = err.to_s
          error_log_text = "MARC parsing failed: #{curation_concern.source_metadata_identifier}: #{err}"
        else
          error_ui_text = 'An unexpected error occurred.'
          error_log_text = "Unexpected error type #{err.class}: #{curation_concern.source_metadata_identifier}: #{err}"
        end
        error_log_text ||= ''
